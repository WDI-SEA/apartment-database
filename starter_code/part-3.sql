...
-- 7. Change Jane's age to 30.
    UPDATE owners SET age = 30 WHERE name = 'Joan';
-- 8. Change Jane's name to Janet.
    UPDATE owners SET name = 'Janet' WHERE name = 'Joan';
-- 9. Delete the owner named Janet.
    DELETE FROM owners WHERE name = 'Janet'
-- 10. Show the names of the first three owners in your owners table.
    SELECT name FROM owners WHERE name IN ('Donald', 'John', 'Yuki');
-- 11. Show the name of all owners whose name contains an `a`.
    SELECT * FROM owners WHERE name LIKE '%a%';
-- 12. Show all of the properties in alphabetical order that are not named Archstone and do not have an id of 3 or 5.
    SELECT name FROM properties WHERE NOT name = 'Archstone' AND NOT (id = 3 OR id = 5) ORDER BY name 
...
-- 14. Show the highest age of owners who are under 30 and whose name contains an `o`. Limit to one result.
    SELECT * FROM owners WHERE name LIKE '%o%' AND age < 30;
