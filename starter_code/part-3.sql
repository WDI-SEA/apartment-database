...
-- nobody has a name that starts with an E
...
SELECT * FROM property, owner WHERE owner.id = property.owner_id ORDER BY owner.name;
...
SELECT COUNT (*) FROM property WHERE owner_id > 0 AND owner_id < 4;
-- Janet was removed, she would be 3 so 2 is returned
